          ...
          <section key={section.id}>
            ...
            <div className="space-y-6">
              {(section.blocks || []).map((block) => {
                switch (block.type) {
                  case "text":
                    return <div key={block.id} className="text-gray-800 leading-7 break-words prose prose-indigo max-w-none [&_ul]:italic [&_ul]:list-disc [&_ul]:pl-6 [&_ol]:list-decimal [&_ol]:pl-6" dangerouslySetInnerHTML={{ __html: block.value }} />;
                  case "image": {
                    let src = block.value;
